...
class DSYMSearch {
    ...
    static func search(forUUID uuid: String, crashFileDirectory: String, fileSearchErrorHandler: @escaping ErrorHandler, completion: @escaping CompletionHandler) {
        ...
        SpotlightSearch.shared.search(forPredicate: predicate) { (results) in
            ...
            _ = results?.first(where: { (metadataItem) -> Bool in
        ...
        if let results = FileSearch.search(fileExtension: "dsym", directory: crashFileDirectory, recursive: false) {
